...
def MyGridSearch(X,y):
    ...
    C_range = np.linspace(10,1e5,5 )
    gamma_range = np.linspace(1e-2, 1e-4, 5)
    epsilon_range = np.array([0.1])

    #epsilon_range = np.linspace(1e-3,1e-7,10)
    ...
    grid = GridSearchCV(model,parameters,cv=5,verbose=10,n_jobs=8)
