...
###############################################################################
kf_total = cross_validation.KFold(len(X), n_folds=5, indices=True, shuffle=True, random_state=4)
for train, test in kf_total:
	X_train, X_test, y_train, y_test = X[train], X[test], y[train], y[test]
	svr_rbf = SVR(kernel='rbf', C=1e3, gamma=0.1)
	svr_rbf.fit(X_train,y_train)
	scores_got=cross_validation.cross_val_score(svr_rbf, X_test, y_test, cv=kf_total, n_jobs = 1)
