...
###avf_file='../dcache_avf_579.txt'
avf_file='../dcache_avf_bug_recovered.txt'
#avf_file='../normalized_dcache_579.txt'
...
for indx,value in np.ndenumerate(X_total):
        X_total[indx]=(X_total[indx]*100)%modulo
...
def MyGridSearch(X,y):
    ...
    #epsilon_range = np.linspace(1e-3,1e-7,5)
...
for loop_iter in range(0, 1): #
    ...
    X_train, X_test, y_train, y_test = cross_validation.train_test_split(X_total, y_total, test_size=0.5, random_state = 42)
