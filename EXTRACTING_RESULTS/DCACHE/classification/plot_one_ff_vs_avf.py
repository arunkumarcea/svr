...
#bs = SelectKBest(f_regression, k=4)
#X_total = bs.fit_transform(X_total, y_total)
X_total_ff1 = X_total[:,[2211]]
X_total_ff2 = X_total[:,[2212]]
X_total_ff3 = X_total[:,[2232]]
X_total_ff4 = X_total[:,[2233]]

div=100/(len(y_total)*1.000)
chumma=np.arange(0, 100,div )
chumma=chumma[:len(y_total)]
#print X_total
#print X_total.shape
plt.hold('on')
plt.plot(chumma, X_total_ff1,'g-', label='sp of FF_rank1')
#plt.plot(chumma, X_total_ff2,'y', label='SVM model')
#plt.plot(chumma, X_total_ff3,'b', label='SVM model')
#plt.plot(chumma, X_total_ff4,'m', label='SVM model')

plt.plot(chumma, y_total, 'r', label='AVF')

plt.xlabel('#normalized number of datapoints/workloads', fontsize=20)
plt.ylabel('AVF/sp', fontsize=20)
plt.title('AVF and Single flipflop sp', fontsize=20)
plt.legend()
plt.show()
